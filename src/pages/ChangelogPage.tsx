...
const LOGS = [
  {
    ...
    title: '数据修复：旧版打卡记录找回',
    items: [
      '修复：v0.1.0 期间的打卡记录（旧存储键 xow_sop_records）在 v0.2.0 升级后不再显示的问题——启动时自动迁移旧记录并映射到新 Checklist 条目（擦亮→B1、上新→B2、回复咨询→A1、议单催拍→A3、订单跟进→A4、竞品观察→C2、数据记录→D1）',
      '设置页新增「数据诊断」（管理员）：各存储计数（用户/打卡记录/经营数据/任务/孤儿记录），孤儿记录一键重新指定归属用户',
      '数据隔离说明：本地 localhost:5181 与线上 github.io 是两套独立 localStorage 数据，互不相通；同一地址换浏览器或清缓存也会分离',
    ],
  },
  {
    version: 'v0.3.0',
